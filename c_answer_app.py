...
def clean_text(text):
	if not text: return ""
	# Strip table artifacts (pipes, long lines of dashes or equals)
	text = text.replace('|', '').replace('---', '').replace('===', '') 
	# Strip markdown headers/bolding
...
def create_pdf(saved_trials, patient_info, treatment_report, comparison_report):
	...
	# 2. Comparison (Now clean text lists)
	if comparison_report:
		...
		# Using multi_cell to handle the new list format
