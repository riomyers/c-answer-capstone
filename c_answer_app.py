...
def clean_text(text):
    """
    Ensures safe text for FPDF generation.
    """
    if not text:
        return ""
    ...
    # ASCII Normalization
    safe_str = unicodedata.normalize('NFKD', text).encode('ascii', 'ignore').decode('ascii')
    return safe_str
