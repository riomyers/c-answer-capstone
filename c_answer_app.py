...
def clean_text(text):
	"""
	Ultimate cleaning function to ensure NO question marks or encoding errors in PDF.
	"""
	if not text:
		return ""
	
	# Standardize whitespace and remove hidden characters
	text = text.replace('\u00A0', ' ').replace('\u200B', '').replace('\u200D', '')
	
	# Aggressive character replacement list
	replacements = {
		'•': '-', '‣': '-', '⁃': '-', '◦': '-',  # Fancy bullets to hyphens
		'—': '-', '–': '-', '\u2013': '-', '\u2014': '-', # Em/En dashes
		'‘': "'", '’': "'", '‚': "'", '‛': "'", # Smart single quotes
		'“': '"', '”': '"', '„': '"', '‟': '"', # Smart double quotes
		'…': '...', '\u2026': '...',             # Ellipsis
	...
	# Remove Markdown headers/bolding artifacts
	text = text.replace('###', '').replace('##', '').replace('#', '').replace('**', '').replace('__', '')
	
	# Final ASCII Normalization (The "Safe Guard")
...
def create_pdf(saved_trials, patient_info, treatment_report, comparison_report):
	...
	# 2. Comparison (RESTORED MATRIX LOGIC)
...
with tab_saved:
	...
	if saved:
		...
		# RESTORED: Comparison Logic with Rerun
		if len(saved) > 1:
			if st.button("⚖️ Compare Selected Trials (AI)", type="primary"):
				with st.spinner("Generating comparison matrix..."):
					...
					st.rerun() # FORCE RERUN TO LOCK IN DATA
		...
		# Always generate bytes based on current state
		...
		st.info("Your shortlist empty.")

# --- TAB 4: ROADMAP ---
with tab_roadmap:
	st.markdown("### 🚀 C-Answer: Phase 2 Roadmap")
	st.write("Our current MVP focuses on analysis. Phase 2 focuses on integration.")
