...
def clean_text(text):
	"""Aggressive ASCII cleaning for PDF text encoding safety."""
	if not text:
		return ""
	...
	# Explicit typographical replacements
	replacements = {
		'\u2018': "'", '\u2019': "'", '\u201c': '"', '\u201d': '"',
		'\u2013': '-', '\u2014': '-', '\u2022': '-', '•': '-',
		'\u2026': '...', '–': '-'
	...
	# Safe encoding lock
	safe_string = unicodedata.normalize('NFKD', text).encode('ascii', 'ignore').decode('ascii')
	return safe_string
...
def create_pdf(saved_trials, patient_info, treatment_report, comparison_report):
	"""Generates the professional clinical report."""
	pdf = FPDF()
...
def render_trial_card(trial):
	"""Renders a single trial card with navigation and AI hooks."""
...
with st.sidebar:
	...
	st.markdown("""
		...
		**HIPAA Notice:** Compliant local session processing.
		""")
...
with tab_search:
			...
			with st.spinner("Extracting clinical data..."):
				try:
					...
					if ext:
						...
						if e_age and str(e_age).isdigit():
							st.session_state.form_age = int(e_age)
						else:
							st.session_state.form_age = None
	...
	with st.expander("Configure Patient Profile", expanded=is_expanded):
		with st.form("patient_form"):
			...
			form_c4, form_c5, form_c6 = st.columns(3)
			with form_c4: 
			...
			form_c7, form_c8, _ = st.columns([1, 1, 2])
			with form_c7: 
...
with tab_insights:
	if st.session_state.treatment_report:
		...
		st.write("👈 Perform a search in the 'Trial Search' tab to generate a treatment report.")
...
with tab_saved:
	...
	if saved:
		...
		# RESTORED INSTRUCTIONS FOR THE EMPTY STATE
...
with tab_roadmap:
	...
	with st.expander("💊 Feature 1: Drug Interaction Safety Check (OpenFDA)", expanded=True):
		st.markdown("""
		**The Problem:** Interference with current meds.
		**The Solution:** OpenFDA API integration to flag contraindications automatically.
		""")
		
	with st.expander("📚 Feature 2: 'True' Live RAG Database", expanded=True):
		st.markdown("""
		**The Problem:** LLM knowledge cutoff.
		**The Solution:** Daily ingestion of live NCCN PDF guidelines into a Vector Database.
		""")
		
	with st.expander("⏰ Feature 3: 24/7 Monitor & Alert System", expanded=True):
		st.markdown("""
		**The Problem:** Manual checking is tedious.
		**The Solution:** Active cloud monitoring with automated email alerts when new trials open.
		""")

# --- MAIN FOOTER ---
st.markdown("<div class='footer-note'>🔒 <b>Privacy Notice:</b> C-Answer is a local-first session-based tool. No patient data, medical records, or results are stored on our servers. Closing this tab permanently clears all information.</div>", unsafe_allow_html=True)
