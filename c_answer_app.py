...
with st.sidebar:
    ...
    with st.form("patient_form"):
        ...
        with col1:
            # CHANGED: value=None makes it empty by default
            age = st.number_input("Age", value=None, placeholder="e.g. 45", step=1)
        with col2:
            # CHANGED: Added "Select..." as the first option
        ...
        with st.expander(f"{title}", expanded=should_be_expanded):
            ...
            with c2:
                ...
                if st.button(f"Analyze Match for {nct_id}", key=f"btn_{nct_id}"):
                    ...
                    # LOGIC CHANGE: Handle empty demographics gracefully
